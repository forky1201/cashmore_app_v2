...
class MissionNormalView extends GetView<MissionController> {
  const MissionNormalView({super.key});
            ...
            Expanded(
              child: Obx(() {
                return RefreshIndicator(
                  onRefresh: () async {
                    await controller.refreshNormalMissions(); // 새로고침 호출
                  },
                  child: controller.isLoadingNormal.value && controller.normalMissions.isEmpty
                      ? const Center(child: CircularProgressIndicator())
                      : controller.normalMissions.isEmpty
                          ? ListView(
                          ...
                          : ListView.builder(
                              ...
                              controller: controller.normalScrollController, // 스크롤 컨트롤러
                              padding: const EdgeInsets.symmetric(horizontal: 16.0),
                              itemCount: controller.normalMissions.length + 1, // 로딩 인디케이터를 위해 +1
                              itemBuilder: (context, index) {
                                if (index == controller.normalMissions.length) {
                                  // 리스트의 끝에 로딩 인디케이터 표시
                                  return controller.isLoadingMoreNormal.value
                                      ? const Padding(
                                ...
                                final mission = controller.normalMissions[index];
                                return GestureDetector(
                                  onTap: () {
                                    // 리스트 항목을 클릭하면 미션 상세 페이지로 이동
                                    controller.moveToUrlNormal("/mission_normal", mission.quest_number);
                                  },
                                  child: _buildMissionListItem(
                                    "일반 퀘스트",
                                    //mission.decode.toString(),
    ...
    return Container(
      ...
      child: Row(
        children: [
          // 아이콘
          Image.asset(
            'assets/images/mission_item_2.png', // PNG 파일 경로
            width: 48, // 적절한 크기 설정
            height: 48,
